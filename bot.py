        ...
        try:
            ...
            for e in parsed.entries:
                ...
                items.append({
                    ...
                    "link": link,
                    "source": feed["name"],
                    "always": feed.get("always_relevant", False),
                })
...
def ai_message(title, text, ai_cfg):
    ...
    prompt = (
        f"אתה עורך תוכן לערוץ טלגרם בעברית בנושא המשחק {team} (League of Legends). בהינתן כותרת וטקסט של כתבה (ייתכן באנגלית): "
        "אם הכתבה אינה עוסקת ב-League of Legends (למשל משחק אחר כמו Valorant/TFT או נושא לא רלוונטי), החזר בדיוק את המילה SKIP וכלום מלבדה. "
        "אחרת, כתוב הודעה קצרה וקולעת בעברית (תרגם לעברית במידת הצורך): שורה ראשונה = כותרת קליטה שמתחילה באימוג'י אחד מתאים, "
